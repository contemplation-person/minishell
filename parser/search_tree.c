...
/*   Updated: 2022/12/30 15:02:01 by gyim             ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
int	search_tree(t_tree_node *node, t_fds *fd_info, t_env_info_list *env_list)
{
	if (node->op != NULL)
	{
		if (ft_strncmp(node->op->token, "|", 2) == 0)
			...
			return (op_infile(node, fd_info, env_list));
		else if (ft_strncmp(node->op->token, ">", 2) == 0)
			return (op_outfile(node, fd_info, env_list));
		else if (ft_strncmp(node->op->token, ">>", 3) == 0)
			return (op_append(node, fd_info, env_list));
		else if (ft_strncmp(node->op->token, "<<", 3) == 0)
			return (op_here_doc(node, fd_info, env_list));
		else if (ft_strncmp(node->op->token, "&&", 3) == 0)
