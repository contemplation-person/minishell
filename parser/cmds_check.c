...
/*   Updated: 2022/12/30 15:05:06 by gyim             ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
int	multiple_cmds_excute(t_tlist_info *word_list, t_env_info_list *env_list)
{
	t_fds		fds;
	t_tree_node	*root;
	int			exit_status;
	...
	if (root)
	{
		if (tree_valid_check(root) != -1)
		{
			exit_status = search_tree(root, &fds, env_list);
			wait(0);
		}
		del_tree(root);
		free(root);
