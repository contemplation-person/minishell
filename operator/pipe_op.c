...
/*   Updated: 2023/01/07 15:59:27 by gyim             ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
int	op_pipe(t_tree_node *node, t_fds *fd_info, t_env_info_list *env_list)
{
	...
	g_error_code = op_pipe_left_excute(node, &child_fds[0], fd, env_list);
	g_error_code = op_pipe_right_excute(node, &child_fds[1], fd, env_list);
	waitpid(0, &status, 0);
	close(fd[1]);
	close(fd[0]);
	close(fd_info->in_fd);
